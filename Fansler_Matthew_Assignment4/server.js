...
app.get('/purchase', function (request, response, _next) { //get data from /purchase action
    ...
    var minMiles = 5; //Minimum charge is 5 miles, number is set to appear automatically in /product_data.js default
    tot_qty = 0;
    for (i = 0; i < products.length; i++) { //starting at 0 then increase by one for every product available
        index = 'qtyTextbox' + i;
        console.log(tot_qty);
        console.log(a_qty);
